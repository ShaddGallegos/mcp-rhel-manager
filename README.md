...
CROSS-PLATFORM POLICY NOTICE: This repository now enforces a RHEL-only development
and managed-node policy. All tooling and CI are targeted for RHEL 8/9/10.
- Use `podman` (not Docker) for container builds and runtime.
- Use `registry.redhat.io` as the preferred container image registry for base images.
- During install, you are prompted up front to authenticate with `podman login registry.redhat.io` using your Red Hat credentials.
- CI workflows were updated to run on self-hosted RHEL runners; avoid `ubuntu-latest`.

Run `./install_system.sh --dry-run` to preview platform-specific steps for RHEL.
...
This quick start is a consolidated, canonical onboarding covering installation,
configuration, and common runtime tasks. For quick templated host-specific
instructions use `scripts/render_quickstart.py`.

### 1. Install and bootstrap

Syntax
```bash
./install_system.sh [--apply|--dry-run] [--start] [--background-only|--eager-start] [--verify] [--verify-json] [--yes] [--force] [--venv] [--base-dir PATH] [--user NAME] [--home PATH] [--selinux permissive|enforcing|unchanged] [--firewalld enabled|disabled|unchanged]
```
...
# Full unattended install with default background-only activation
./install_system.sh --start --yes

# Full unattended install with MCP background-only activation (default)
./install_system.sh --start --background-only --yes

# Full unattended install with immediate MCP starts
./install_system.sh --start --eager-start --yes
...
| `HAL --doctor`            | Check runtime deps, writable paths, bridge, and Ollama    |
