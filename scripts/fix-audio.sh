...
if [ "$IS_WSLG" -eq 1 ]; then
  ...
  if wait_for_service wireplumber.service active 8; then
    ok "WirePlumber active"
  else
    fail "WirePlumber failed to start — run: journalctl --user -u wireplumber -n 30"
    exit 1
  fi
  ...
  systemctl --user start pipewire.socket pipewire.service
  if wait_for_service pipewire.service active 8; then
    ok "PipeWire active"
  else
    fail "PipeWire failed to start — run: journalctl --user -u pipewire -n 30"
    exit 1
  fi
  ...
  if wait_for_service pipewire-pulse.service active 8; then
