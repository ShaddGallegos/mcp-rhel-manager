...
def cmd_configure(setting_args: list[str]) -> int:
    ...
    if setting == 'hal-quotes':
        ...
        if not enabled and not disabled:
            print(f'Unknown value "{value_str}". Use: true or no')
            return 2
        val = enabled
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ai.add_argument('--reaper', nargs=argparse.REMAINDER, metavar='REAPER_ARGS',
                    help='Run integrated Reaper CLI (pass subcommand and its args)')
...
def main():
    ...
    if args.reaper:
        # Forward remaining args to the Reaper CLI wrapper
        try:
            from scripts.reaper import reaper_cli
        except Exception as e:
            print('Reaper integration not available:', e, file=sys.stderr)
            return 2
        try:
            reaper_cli.main(args.reaper)
