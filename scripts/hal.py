...
class _IngestCommonStub:
    @staticmethod
    def append_jsonl(*_args, **_kwargs) -> None:
        return None
...
try:
    ...
    ingest_common = _IngestCommonStub()
...
cfg = None
try:
    cfg_spec = importlib.util.spec_from_file_location('mcp_config', os.path.join(REPO_ROOT, 'scripts', 'mcp_config.py'))
    if cfg_spec is not None and cfg_spec.loader is not None:
...
if cfg is not None:
    ...
    MCP_HOME = os.environ.get('HAL_MCP_HOME', getattr(cfg, 'MCP_HOME', MCP_HOME))
...
class HALRuntimeMode:
    conversational: bool = True


HAL_RUNTIME = HALRuntimeMode(conversational=_env_flag('HAL_CONVERSATIONAL', True))


def _build_runtime_mode(args: argparse.Namespace) -> HALRuntimeMode:
    """Build validated per-process runtime mode flags.

    The environment controls default behavior; CLI modes like interactive and
    voice-chat imply conversational behavior for that invocation.
    """
    conversational = _env_flag('HAL_CONVERSATIONAL', True)
    if getattr(args, 'interactive', False) or getattr(args, 'voice_chat', False):
        conversational = True
    return HALRuntimeMode(conversational=conversational)
...
def _print_and_record_response(user: str, text: str, response_text: str, *, remediate: bool = False, exec_allowed: bool = False) -> str:
    print('\nHAL response:\n')
    print(response_text)
    entry_path = write_interaction(user, text, response_text)
    print('\nInteraction recorded ->', entry_path)
    if remediate:
        invoke_remediator(entry_path, exec_allowed)
    return entry_path


def _collect_local_diagnostics_json() -> tuple[dict | None, str]:
    try:
    ...
    try:
        return json.loads(full_diag), full_diag
    except Exception:
        return None, full_diag


def _run_health_report_flow(user: str, text: str, args: argparse.Namespace, diag_json: dict) -> None:
    ...
    if not hw and not sec:
        ...
        _print_and_record_response(user, text, well, remediate=args.remediate, exec_allowed=args.exec)
        sys.exit(0)
...
def _run_doctor_report() -> tuple[str, int]:
    lines: list[str] = []
    failures = 0

    lines.append('HAL Doctor')
    lines.append('=' * 80)
    ...
    try:
        ...
        _status(True, 'urllib3', getattr(urllib3, '__version__', 'unknown'))
    except Exception as exc:
        _status(False, 'urllib3', str(exc))

    try:
        import chardet  # type: ignore
        _status(True, 'chardet', getattr(chardet, '__version__', 'unknown'))
    except Exception as exc:
        _status(False, 'chardet', str(exc))

    try:
        import charset_normalizer  # type: ignore
        _status(True, 'charset_normalizer', getattr(charset_normalizer, '__version__', 'unknown'))
    except Exception as exc:
        _status(False, 'charset_normalizer', str(exc))

    if requests is not None:
        _status(True, 'requests', getattr(requests, '__version__', 'unknown'))
    else:
        _status(False, 'requests', 'module failed to import')

    for label, path in [('AI_HOME', AI_HOME), ('TRAIN_DIR', TRAIN_DIR), ('FIXES_DIR', FIXES_DIR), ('REPORTS_DIR', REPORTS_DIR), ('CACHE_DIR', CACHE_DIR)]:
        ok = os.path.isdir(path) and os.access(path, os.W_OK)
        detail = path if ok else f'{path} (missing or not writable)'
        _status(ok, label, detail)

    try:
        import urllib.request
        with urllib.request.urlopen('http://localhost:11434/api/tags', timeout=5) as resp:
            _status(True, 'ollama', f'HTTP {resp.status}')
    except Exception as exc:
        _status(False, 'ollama', str(exc))

    try:
        import urllib.request
        with urllib.request.urlopen('http://localhost:1776/health', timeout=5) as resp:
            _status(True, 'bridge', f'HTTP {resp.status}')
    except Exception as exc:
        _status(False, 'bridge', str(exc))
...
def launch_menu(auto_choice: str | None = None) -> None:
    """Minimal interactive menu for common HAL actions.

    If `auto_choice` is provided, run that single action then return.
    """
    import getpass
    ...
    def _call_script(*flags: str) -> None:
        try:
            cmd = [sys.executable, os.path.join(BASE_DIR, 'scripts', 'hal.py'), *flags]
            subprocess.run(cmd)
        except Exception as e:
            print('Failed to run command:', e, file=sys.stderr)

    def _run_dashboard() -> None:
        dash = os.path.join(BASE_DIR, 'scripts', 'hal-dashboard.py')
        if os.path.exists(dash):
            ...
            print('hal-dashboard not found at', dash)

    def _start_llm() -> None:
        script = os.path.join(BASE_DIR, 'scripts', 'start_llm_container.sh')
        try:
            if os.path.exists(script):
                logf = os.path.join(REPORTS_DIR, 'start_llm.log')
                os.makedirs(os.path.dirname(logf), exist_ok=True)
                p = subprocess.Popen(['bash', script], stdout=open(logf, 'a'), stderr=subprocess.STDOUT, start_new_session=True)
                print('LLM helper launched (pid=%s)' % p.pid)
            else:
                ...
                if docker:
                    ...
                    if res.returncode == 0:
                        print('LLM container started:', res.stdout.strip())
                    else:
                        print('Failed to start LLM container:', res.stderr.strip())
                else:
                    print('No LLM helper script or docker/podman found')
        except Exception as e:
            print('Start LLM failed:', e)

    def _start_queue() -> None:
        script = os.path.join(BASE_DIR, 'scripts', 'mcp_inference_queue.py')
        try:
            if os.path.exists(script):
                logf = os.path.join(REPORTS_DIR, 'mcp_inference_queue.log')
                os.makedirs(os.path.dirname(logf), exist_ok=True)
                p = subprocess.Popen([sys.executable, script], stdout=open(logf, 'a'), stderr=subprocess.STDOUT, start_new_session=True)
                print('Inference queue started (pid=%s)' % p.pid)
            else:
                print('Queue script not found at', script)
        except Exception as e:
            print('Start queue failed:', e)
    ...
    def _start_redis() -> None:
        try:
            ...
            if docker:
                ...
                if res.returncode == 0:
                    print('Redis container started:', res.stdout.strip())
                else:
                    print('Failed to start redis:', res.stderr.strip())
            else:
                print('docker/podman not found; cannot start Redis container')
        except Exception as e:
            print('Start redis failed:', e)

    def _start_queue_with_redis() -> None:
        try:
            _start_redis()
            script = os.path.join(BASE_DIR, 'scripts', 'mcp_inference_queue.py')
            if os.path.exists(script):
                ...
                logf = os.path.join(REPORTS_DIR, 'mcp_inference_queue_redis.log')
                os.makedirs(os.path.dirname(logf), exist_ok=True)
                p = subprocess.Popen([sys.executable, script], env=env, stdout=open(logf, 'a'), stderr=subprocess.STDOUT, start_new_session=True)
                print('Inference queue started with REDIS_URL (pid=%s)' % p.pid)
            else:
                print('Queue script not found at', script)
        except Exception as e:
            print('Start queue with redis failed:', e)

    def _switch_default_model() -> None:
        """Interactively choose and persist a default Ollama model for HAL.

        Writes `default_model` into `~/.mcp-ai/hal-config.json` and sets
        `HAL_FORCE_MODEL` in the current environment for immediate effect.
        """
        try:
        ...
        if models:
            ...
            print('  (No models reported by local bridge)')

        print('\nEnter a number to select a model, a model name, or `unset` to remove default. Type `q` to cancel.')
        sel = input('model> ').strip()
        if not sel or sel.lower() in ('q', 'quit', 'exit'):
            print('Cancelled.')
            return

        chosen = None
        if sel.lower() in ('unset', 'none'):
            chosen = None
        else:
            # numeric selection
            try:
        ...
        cfg_path = os.path.join(AI_HOME, 'hal-config.json')
        cfg = {}
        try:
        ...
        if chosen is None:
            if 'default_model' in cfg:
                ...
                try:
                    ...
                    pass
            os.environ.pop('HAL_FORCE_MODEL', None)
            print('Default model unset; HAL will auto-select next time.')
            return
        ...
        try:
            ...
            with open(cfg_path, 'w', encoding='utf-8') as fh:
                json.dump(cfg, fh, indent=2)
            os.environ['HAL_FORCE_MODEL'] = chosen
            print(f"Default model set to: {chosen}")
        except Exception as e:
            print('Failed to persist default model:', e)

    actions: dict[str, tuple[str, Callable]] = {
        '1': ('Interactive REPL', lambda: _run_interactive_repl(user)),
        '2': ('Voice chat', lambda: _run_voice_chat(user)),
        '3': ('HAL Dashboard (web)', _run_dashboard),
        '4': ('Launch TUI', lambda: subprocess.run([sys.executable, os.path.join(BASE_DIR, 'scripts', 'hal_tui.py')])),
        '5': ('List models', lambda: _call_script('--model-list')),
        '10': ('Switch default model', _switch_default_model),
        '6': ('Start LLM container', _start_llm),
        '7': ('Start inference queue', _start_queue),
        '8': ('Start Redis (docker)', _start_redis),
        '9': ('Start queue wired to Redis', _start_queue_with_redis),
        'q': ('Quit', lambda: None),
    }

    def _print_menu() -> None:
        print('\nHAL Menu — select an option:')
        for k, v in actions.items():
            print(f'  {k}) {v[0]}')

    if auto_choice:
        choice = (auto_choice or '').strip().lower()
        if choice in actions:
            try:
                actions[choice][1]()
            except Exception as e:
                print('Menu action failed:', e)
        else:
            print('Unknown menu choice:', auto_choice)
        return
...
def _update_session_analytics(query: str, model: str) -> None:
    ...
    try:
        ...
        if _HAS_INGEST_COMMON:
            try:
                ingest_common.append_jsonl(log_path, entry)
            except Exception:
...
def _load_module_from_path(module_name: str, module_path: str):
    """Best-effort dynamic module loading from an absolute file path."""
...
def _call_local_ollama(prompt: str, timeout: int = 45) -> str | None:
    """Call a local Ollama endpoint for lightweight offline enhancement."""
    if not prompt or requests is None:
        return None

    url = os.environ.get('HAL_LOCAL_OLLAMA_URL', 'http://localhost:11434/api/generate')
    try:
        timeout = int(os.environ.get('HAL_LOCAL_OLLAMA_TIMEOUT_SEC', str(timeout)))
    except Exception:
        timeout = timeout
    try:
        max_prompt_chars = int(os.environ.get('HAL_LOCAL_OLLAMA_MAX_PROMPT_CHARS', '12000'))
    except Exception:
        max_prompt_chars = 12000
    model = (
        os.environ.get('HAL_OFFLINE_OLLAMA_MODEL')
...
def _answer_with_local_model(text: str, rag_context: str | None = None) -> str | None:
    """Attempt to answer using a local model.

    Minimal safe stub: return None when no local model is configured. This
    avoids NameError in environments where local-model helpers are not
    implemented or intentionally disabled (tests may monkeypatch higher-level
    call sites instead).
    """
    try:
        return None
...
def _log_interaction(entry: dict) -> None:
    ...
    try:
        ...
        try:
            if _HAS_INGEST_COMMON:
                try:
                    ingest_common.append_jsonl(p, entry)
                except Exception:
...
def write_interaction(user, request_text, response_text):
    ...
    # Optionally trigger autonomous auto-improve scanner in background.
    try:
        if os.environ.get('HAL_AUTONOMOUS_IMPROVE', '').lower() in ('1', 'true', 'yes'):
            resp_text = response_text if isinstance(response_text, str) else str(response_text)
            probe = os.environ.get('HAL_AUTONOMOUS_IMPROVE_PROB', '0.02')
            try:
                prob = float(probe)
            except Exception:
                prob = 0.02
            keywords = ('recommend', 'improve', 'suggest', 'proposal', 'install', 'upgrade', 'add', 'fix', 'patch', 'enhance')
            trigger = any(k in (resp_text or '').lower() for k in keywords) or (random.random() < prob)
            if trigger:
                cmd = [sys.executable, os.path.join(BASE_DIR, 'scripts', 'hal_auto_improve.py'), '--root', BASE_DIR, '--max-files', os.environ.get('HAL_AUTO_IMPROVE_MAX_FILES', '500')]
                # Always generate patches; only auto-apply when explicitly allowed via env var
                cmd.append('--generate-patches')
                if os.environ.get('HAL_ALLOW_AUTO_IMPROVE_APPLY', '').lower() in ('1', 'true', 'yes'):
                    cmd.append('--autonomous')
                # Launch in background to avoid blocking HAL response
                try:
...
def _privileged_audit_log(entry: dict) -> None:
    ...
    try:
        ...
        try:
            try:
                if _HAS_INGEST_COMMON:
                    ingest_common.append_jsonl(logf, entry_out)
                else:
                    raise Exception('no_ingest')
            except Exception:
...
def _privileged_action_flow(user: str, user_text: str) -> None:
    ...
    try:
        ...
        try:
            ...
            sudo_helper = _load_module_from_path('sudo_helper', os.path.join(BASE_DIR, 'scripts', 'sudo_helper.py'))
            if sudo_helper is None:
                raise ImportError('Unable to load sudo_helper module specification')
...
def _load_model_profile_overrides() -> dict:
    ...
    if not raw:
        ...
        try:
            cfg_mod = globals().get('cfg')
            if cfg_mod is None:
                cfg_mod = _load_module_from_path('mcp_config', os.path.join(BASE_DIR, 'scripts', 'mcp_config.py'))
            if cfg_mod is not None:
                if hasattr(cfg_mod, 'get'):
                    raw = cfg_mod.get('model_overrides', None)
                else:
                    raw = getattr(cfg_mod, 'model_overrides', None)
        except Exception:
            raw = None
    if not raw:
        ...
        try:
            candidates = [
                os.path.join(BASE_DIR, 'config', 'mcp-config.json'),
                os.path.join(BASE_DIR, 'mcp-config.json'),
            ]
...
def choose_model_for_task(text: str | None = None, task_profile: str | None = None, available: list[str] | None = None) -> str:
    ...
    # If not forced by CLI/env, check persistent config (~/.mcp-ai/hal-config.json)
    if not forced_model:
        try:
            cfg_path = os.path.join(AI_HOME, 'hal-config.json')
            if os.path.isfile(cfg_path):
            ...
            if route_when:
                try:
                    ...
                    if os.path.exists(mod_path):
                        moe_mod = _load_module_from_path('moe_router', mod_path)
                        if moe_mod is not None and hasattr(moe_mod, 'moe_call'):
...
def _validate_critical_configs() -> tuple[list[str], list[str]]:
    ...
    mcp_config = next((p for p in mcp_config_candidates if os.path.isfile(p)), mcp_config_candidates[0])
...
def _log_quality_event(user: str, query: str, retried: bool, fallback_used: bool, rag_confidence: str) -> None:
    ...
    try:
        ...
        try:
            if _HAS_INGEST_COMMON:
                try:
                    ingest_common.append_jsonl(out_path, evt)
                except Exception:
...
def _extract_vendors_from_record(record: dict, account: str | None = None, vendor_keywords: list | None = None) -> list[dict]:
    ...
    try:
        ...
        if os.path.exists(bf_path) and account:
            bf = _load_module_from_path('bi_fetcher', bf_path)
            if bf is not None:
...
def _load_hal_notify_module():
    ...
    if not os.path.isfile(mod_path):
        return None
    try:
        mod = _load_module_from_path('hal_notify', mod_path)
        if mod is None:
            return None
        _HAL_NOTIFY_MOD = mod
        return _HAL_NOTIFY_MOD
...
def _convert_public_enrichment_to_business_intel(account_name: str) -> bool:
    ...
    try:
        ...
        try:
            mod = _load_module_from_path('ingest_business_intel', os.path.join(BASE_DIR, 'mcp-ai', 'ingest_business_intel.py'))
            if mod is None:
                return False
        ...
        for contact in contact_list:
            ...
            if email and '@' in email:
                ...
                if details:
                    ...
                    verified_email = details.get('email') or email
                    email_display = verified_email if show_full else _mask_email(verified_email)
                    lines.append(f"    • {name_display} — {email_display} (confidence={conf}){hit_note}")
                    contacts_for_export.append({'name': name_display, 'email': verified_email, 'confidence': conf, 'source': details.get('source_file')})
                else:
            ...
            if os.path.exists(bf_path):
                ...
                if bf is not None:
                    ...
                    for c in ext.get('contacts', [])[:15]:
                        ...
                        email_display = email_val if _show_full_contacts() else _mask_email(email_val)
                        ...
                        contacts_for_export.append({'name': c.get('name',''), 'email': email_val, 'confidence': c.get('verification',{}).get('confidence',''), 'source': c.get('source')})
                        printed += 1
        except Exception:
            pass
...
def main():
    global RUNTIME_FORCE_MODEL, RUNTIME_FORCE_PROFILE, HAL_RUNTIME
    ...
    ap.add_argument('--doctor', action='store_true', help='Run HAL runtime environment checks (deps, bridge, paths)')
    ...
    args = ap.parse_args()
    HAL_RUNTIME = _build_runtime_mode(args)

    # Quick 'hal menu' shortcut: `hal menu` or `hal menu <option>`
    if getattr(args, 'text', None):
        if len(args.text) >= 1 and str(args.text[0]).strip().lower() == 'menu':
            auto_choice = str(args.text[1]).strip().lower() if len(args.text) > 1 else None
            launch_menu(auto_choice)
            sys.exit(0)
    ...
    if args.predict:
        try:
            ...
            hal_metrics = _load_module_from_path('hal_metrics', metrics_path)
            if hal_metrics is None:
    ...
    if not args.text and not args.diagnostics and not args.status and not args.interactive and not args.inventory and not args.metrics and not args.predict and not args.studio and not args.studio_pipeline and not args.doctor:
        ap.print_help()
        sys.exit(0)
    ...
    if args.doctor:
        report, exit_code = _run_doctor_report()
        print(report)
        sys.exit(exit_code)
    ...
    try:
        if _is_well_query(text):
            ...
            if HAL_RUNTIME.conversational:
                ...
                if ans.startswith('y'):
                    diag_json, full_diag = _collect_local_diagnostics_json()
                    if diag_json is None and isinstance(full_diag, str) and full_diag.startswith('ERR:'):
                        # fallback to LLM if diagnostics failed
                        resp = call_bridge(text)
                        assistant_text = extract_assistant_content(resp)
                        _print_and_record_response(user, text, assistant_text if assistant_text else resp, remediate=args.remediate, exec_allowed=args.exec)
                        sys.exit(0)
                    _run_health_report_flow(user, text, args, diag_json)
                else:
