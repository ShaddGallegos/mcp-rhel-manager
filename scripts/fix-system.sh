...
if [[ $EUID -eq 0 ]]; then
  ...
  if ! sudo -n true 2>/dev/null; then
    ...
    sudo true || {
      fail "Could not obtain sudo. Aborting."
      exit 1
    }
  fi
fi
...
if [[ $AS_ROOT -eq 1 ]]; then
  ...
  if [[ $IS_WSLG -eq 1 ]]; then
    ...
    if wait_for_svc wireplumber.service active 8; then
    ...
    if wait_for_svc pipewire.service active 8; then
      ok "PipeWire active"
    else
    ...
    if wait_for_svc pipewire-pulse.service active 8; then
...
if [[ $AS_ROOT -eq 1 ]]; then
  ...
  if [[ -f $BRIDGE_SVC ]]; then
    ...
    if [[ -n $BRIDGE_CFG && ! -f $BRIDGE_CFG ]]; then
      ...
      for candidate in "${MCP_BASE}/config/mcp-config.json" \
        "${MCP_BASE}/mcp-config.json" \
        "${HOME}/mcp-rhel-manager/config/mcp-config.json" \
    ...
    if [[ -x $BRIDGE_PYTHON ]]; then
      if ! "$BRIDGE_PYTHON" -c "import ollama_mcp_bridge" 2>/dev/null; then
        ...
        if [[ -n $REQS ]]; then
          if "${MCP_BASE}/venv/bin/pip" install -q -r "$REQS" 2>/dev/null; then
          ...
          # Try direct install
          if "${MCP_BASE}/venv/bin/pip" install -q ollama-mcp-bridge 2>/dev/null; then
...
if ! sudo ls "${EFI_KVER_DIR}/" 2>/dev/null | grep -q "linux\|vmlinuz\|initrd"; then
  ...
  if [[ -f $VMLINUZ ]]; then
    if [[ ! -f $INITRAMFS ]]; then
      ...
      if sudo dracut --force "$INITRAMFS" "$KVER" 2>/dev/null; then
        fixed "Generated initramfs: $INITRAMFS"
      else
        warn "dracut failed — boot may still work via existing grub entries"
      fi
    fi
    if sudo kernel-install add "$KVER" "$VMLINUZ" "$INITRAMFS" 2>/dev/null; then
...
if sudo test -d "$PYLIB_PATH" 2>/dev/null; then
  ...
  if [[ -n $WRONG ]]; then
    ...
    if sudo restorecon -RF "$PYLIB_PATH" 2>/dev/null; then
