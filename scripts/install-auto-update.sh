...
if [ ! -f "$REPO_ROOT/systemd/hal-auto-update.service" ]; then
    echo "ERROR: hal-auto-update.service not found in $REPO_ROOT/systemd"
    exit 1
fi
if [ ! -f "$REPO_ROOT/systemd/hal-auto-update.timer" ]; then
    echo "ERROR: hal-auto-update.timer not found in $REPO_ROOT/systemd"
    exit 1
fi

cp "$REPO_ROOT/systemd/hal-auto-update.service" /etc/systemd/system/
cp "$REPO_ROOT/systemd/hal-auto-update.timer" /etc/systemd/system/
