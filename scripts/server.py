import sys, os, subprocess, psutil, platform, shutil, json, time, socket, hashlib, shlex
...
def _load_server_config() -> dict:
    ...
    cfg_path = os.path.join(BASE_DIR, 'config', 'mcp-config.json')
...
def _audit_event(event: str, details: dict) -> None:
    try:
        ...
        if _HAS_INGEST_COMMON:
            try:
                ingest_common.append_jsonl(audit_path, payload)
                return
            except Exception:
                pass
...
def _mcp_health_snapshot() -> dict:
    ...
    # Audio diagnostics: lightweight checks (best-effort)
    try:
        script_path = os.path.join(BASE_DIR, 'scripts', 'auto-fixer.sh')
        audio_backend_res = run_cmd_result(['bash', '-c', f'. "{script_path}" && detect_audio_backend'], timeout=6)
        backend = (audio_backend_res.get('stdout') or '').strip() if audio_backend_res.get('ok') else ''

        mic_test_res = run_cmd_result(['bash', '-c', f'. "{script_path}" && test_microphone_recording'], timeout=20)
        mic_rc = mic_test_res.get('rc')
        mic_ok = True if mic_rc == 0 else (False if mic_rc in (1,) else None)

        # Playback device detection: try pactl/pw or aplay
        playback_ok = False
        if shutil.which('pactl'):
            sinks = run_cmd_result(['pactl', 'list', 'short', 'sinks'], timeout=6)
            playback_ok = bool((sinks.get('stdout') or '').strip())
        elif shutil.which('pw-dump') or shutil.which('pw-cli'):
            pd = run_cmd_result(['bash', '-c', 'pw-dump || pw-cli info'], timeout=6)
            playback_ok = bool((pd.get('stdout') or '').strip())
        elif shutil.which('aplay'):
            ap = run_cmd_result(['aplay', '-l'], timeout=6)
            playback_ok = ('card' in (ap.get('stdout') or '').lower()) or bool(ap.get('stdout'))

        checks['audio_backend'] = {'ok': bool(backend and backend != 'none'), 'backend': backend}
        checks['audio_mic'] = {'ok': mic_ok is True, 'rc': mic_rc, 'note': 'None means no test tool available' if mic_ok is None else ''}
        checks['audio_playback'] = {'ok': bool(playback_ok), 'details': 'sinks_found' if playback_ok else 'no_sinks_detected'}
    except Exception:
        # Best-effort only; don't crash health snapshot
        checks.setdefault('audio_backend', {'ok': False, 'backend': ''})
        checks.setdefault('audio_mic', {'ok': False, 'rc': None})
        checks.setdefault('audio_playback', {'ok': False, 'details': 'error'})
...
# -- Audio helpers & MCP tools -------------------------------------------------
def _run_autofixer_func(func_name: str, timeout: int = 60) -> dict:
    """Source the auto-fixer script and invoke a named function (best-effort).

    Returns the structured result from `run_cmd_result`.
    """
    try:
        script_path = os.path.join(BASE_DIR, 'scripts', 'auto-fixer.sh')
        # Use bash -c to source the script and call the function
        cmd = ['bash', '-c', f'. "{script_path}" && {func_name}']
        return run_cmd_result(cmd, timeout=timeout)
    except Exception as exc:
        return {'ok': False, 'rc': None, 'stdout': '', 'stderr': str(exc), 'error': f'ERR: {exc}', 'duration_ms': 0, 'command': cmd}


@mcp.tool()
@_guarded_tool('mcp_audio_check')
def mcp_audio_check():
    """Run a best-effort audio backend, microphone and playback check.

    This is non-destructive and only runs short userland probes if available.
    """
    tool_name = 'mcp_audio_check'
    try:
        script_path = os.path.join(BASE_DIR, 'scripts', 'auto-fixer.sh')
        backend_res = run_cmd_result(['bash', '-c', f'. "{script_path}" && detect_audio_backend'], timeout=6)
        backend = (backend_res.get('stdout') or '').strip() if backend_res.get('ok') else ''

        mic_res = run_cmd_result(['bash', '-c', f'. "{script_path}" && test_microphone_recording'], timeout=20)
        mic_rc = mic_res.get('rc')
        mic_ok = True if mic_rc == 0 else (False if mic_rc in (1,) else None)
        ...
        if shutil.which('pactl'):
            ...
            ap = run_cmd_result(['aplay', '-l'], timeout=6)
            playback_ok = bool((ap.get('stdout') or '').strip())

        data = {
            'backend': backend,
            'microphone': {'ok': mic_ok is True, 'rc': mic_rc, 'stdout_head': (mic_res.get('stdout') or '')[:400]},
            'playback': {'ok': bool(playback_ok)},
        }
        return _tool_ok(tool_name, 'Audio check completed.', data=data, code='AUDIO_CHECK_OK')
    except Exception as exc:
        return _tool_err(tool_name, 'AUDIO_CHECK_FAILED', f'Audio check failed: {exc}', details={}, retryable=True)


@mcp.tool()
@_guarded_tool('mcp_audio_remediate')
def mcp_audio_remediate():
    """Attempt non-destructive audio remediation using bundled auto-fixer logic.

    This sources `scripts/auto-fixer.sh` and invokes `check_and_fix_mic` once.
    """
    tool_name = 'mcp_audio_remediate'
    t = _resolve_timeout(tool_name, None, 120)
    res = _run_autofixer_func('check_and_fix_mic', timeout=t)
    details = _cmd_details('check_and_fix_mic', res)
    if res.get('ok') and res.get('rc') == 0:
        return _tool_ok(tool_name, 'Audio remediation attempted and succeeded.', data=details, code='AUDIO_REMEDIATE_OK')
    # Some scripts return non-zero but may have partial fixes — surface details
    return _tool_err(tool_name, 'AUDIO_REMEDIATE_FAILED', 'Audio remediation failed or incomplete.', details=details, retryable=True, action_hint='Run mcp_audio_check for diagnostics and consider manual intervention.')


def _maybe_auto_fix_audio_on_startup():
    """If configured, run a one-shot audio remediation in background at server start."""
    try:
        health_cfg = SERVER_CONFIG.get('health', {}) or {}
        if not bool(health_cfg.get('autoFixAudioOnBoot', False)):
            return

        def _worker():
            try:
                res = _run_autofixer_func('check_and_fix_mic', timeout=_resolve_timeout('mcp_audio_remediate', None, 120))
                _audit_event('auto-audio-remediate', {'ok': bool(res.get('ok')), 'rc': res.get('rc')})
            except Exception as exc:
                _audit_event('auto-audio-remediate-exception', {'error': str(exc)})
...
# Trigger optional startup remediation
_maybe_auto_fix_audio_on_startup()
...
def mcp_server_capabilities():
    ...
    capabilities = {
        ...
        'timestamp': _utc_now_iso(),
        'config_source': os.path.join(BASE_DIR, 'config', 'mcp-config.json'),
        'features': {
