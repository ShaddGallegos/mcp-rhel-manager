...
mkdir -p ~/mcp-rhel-manager/scripts ~/.config/systemd/user
cd ~/mcp-rhel-manager
...
cat << 'BEOF' > scripts/auto-fixer.sh
#!/bin/bash
echo "MCP Sentinel Active"
exit 0
BEOF
chmod +x scripts/auto-fixer.sh
...
[Service]
Type=simple
ExecStart=/home/sgallego/mcp-rhel-manager/venv-bridge/bin/python /home/sgallego/mcp-rhel-manager/scripts/bridge.py
Restart=on-failure
RestartSec=5
...
[Service]
Type=simple
ExecStart=/home/sgallego/mcp-rhel-manager/scripts/auto-fixer.sh
Restart=on-failure
RestartSec=5
