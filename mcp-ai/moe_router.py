...
def _aggregate(results: dict[str, str], user_text: str, timeout: int = 60, model_map: dict | None = None) -> str:
    ...
    t0 = time.time()
    # Call bridge/caller in a backward-compatible way: some test fakes or
    # older implementations may not accept an `endpoint` kwarg. Use
    # `inspect.signature` to decide how to call `_call_bridge`.
    try:
        import inspect
        sig = inspect.signature(_call_bridge)
        ...
        # Best-effort fallback: try without endpoint first, then with minimal args
        try:
            out = _call_bridge(messages, model=model, timeout=timeout)
        except Exception:
            out = _call_bridge(messages, model=model)
    latency = time.time() - t0
...
def _emit_moe_metric(data: dict) -> None:
    ...
    try:
        ...
        if _HAS_INGEST_COMMON:
            try:
                ingest_common.append_jsonl(path, entry)
            except Exception:
