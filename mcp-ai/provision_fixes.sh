...
# Also copy any top-level repo fix-*.sh or fix_*.sh scripts into the user's fixes dir
for src in "$SCRIPT_DIR/../fix-"*.sh "$SCRIPT_DIR/../fix_"*.sh; do
    [[ -f "$src" ]] || continue
    fn="$(basename "$src")"
    dst="$USER_DIR/$fn"
    if [[ $FORCE -eq 1 ]]; then
        if cp -a "$src" "$dst"; then
            echo "Installed $fn (from repo root)"
        else
            echo "Failed to install $fn"
            continue
        fi
