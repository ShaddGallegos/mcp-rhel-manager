...
def redact_text(s: str) -> str:
    if not isinstance(s, str):
        return s
    # Prefer gstack's richer auto-redaction when available.
    try:
        import ingest_common

        try:
            redacted = ingest_common.call_gstack_auto_redact(s)
            if redacted:
                return redacted
        except Exception:
            # Fall through to local heuristics on any failure.
            pass
    except Exception:
        # ingest_common not available; continue with local heuristics.
        pass
